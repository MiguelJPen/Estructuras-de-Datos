...
public:
	...
	int count_interval(const T &lower, const T &upper) const {
		return empty() ? 0 : cInterval(raiz, lower, upper);
	}
...
protected:
	...
	int cInterval(Link a, T const& lower, T const& upper) const {
		int n = 0;

		if (a == nullptr) return 0;

		if (menor(a->elem, lower))
			n = cInterval(a->dr, lower, upper);
		else if (menor(upper, a->elem))
			n += cInterval(a->iz, lower, upper);
		else {
			++n;
			n += cInterval(a->iz, lower, upper) + cInterval(a->dr, lower, upper);
		}
		return n;
	}
