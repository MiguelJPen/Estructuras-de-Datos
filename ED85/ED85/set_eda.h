...
public:
	...
	std::pair<bool, T> lower_bound(T const& e) const {
		if (menor(e, raiz->elem))
			return lower_bound(e, raiz, true, raiz->elem);
		return lower_bound(e, raiz, false, e);
	}
...
protected:
	...
	std::pair<bool, T> lower_bound(T const& e, Link a, bool hasGreater, T gr) const {
		if (a == nullptr)
			return std::make_pair(false, e);
		else if (menor(e, a->elem)) {
			gr = a->elem;
			if (a->iz != nullptr)
				return lower_bound(e, a->iz, true, gr);
			else return std::make_pair(true, gr);
		}
		else if (menor(a->elem, e)) {
			if (a->dr != nullptr)
				return lower_bound(e, a->dr, hasGreater, gr);
			else return std::make_pair(hasGreater, gr);
		}
		else return std::make_pair(true, e);
	}
